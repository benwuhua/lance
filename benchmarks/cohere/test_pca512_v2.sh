#!/bin/bash
# test_pca512_v2.sh - PCA-512 DRAM + OBS sweep with corrected GT
set -e
...
GT=$PCA_DIR/gt_positional_v2.npz
RESULT_DIR=/data/work/tmp/pca512-results-v2
BENCH="benchmarks/cohere/bench.py"

mkdir -p $RESULT_DIR

echo "================================================================"
echo "  PCA-512 DRAM + OBS Sweep (GT v2)"
...
OBS_SHARD_DIR=s3://knowledgebase-5f43/pca512-1b

run_obs() {
    local np=$1 rf=$2
    local label="pca512-obs-np${np}-rf${rf}"
    echo ""
    echo "  Running: $label"
    python -u $BENCH query \
        --plan B --storage obs --index-type IVF_RQ \
